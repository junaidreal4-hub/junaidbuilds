const projects = [
  ...
  },
  {
    ...
    status: 'Live',
    desc:   '8-page corporate website with Google Sheets API lead capture, interactive data visualisations, SEO optimisation, and custom Vercel deployment.',
  ...
  },
  {
    ...
    tag:    'Full-Stack SaaS',
    status: 'In Progress',
    desc:   'AI-powered job application assistant. Full-stack app with Next.js, FastAPI backend, PostgreSQL, and OpenAI integration for CV and cover letter generation.',
      ...
      <div className="container-width">
        ...
        <div className="divide-y divide-border">
          {projects.map((p) => (
            <div
              key={p.title}
              className="group grid grid-cols-1 md:grid-cols-[80px_1fr_160px] gap-6 md:gap-10 py-12 transition-colors duration-200 hover:bg-canvas md:-mx-6 md:px-6"
            >
              <div className="label pt-1.5">{p.index}</div>

              <div>
                <div className="flex flex-wrap items-center gap-3 mb-4">
                  <h3 className="font-display italic text-[clamp(1.8rem,4vw,3rem)] text-heading leading-none">
                    {p.title}
                  </h3>
                  <span className="label border border-border px-2 py-1">{p.tag}</span>
                  <span className={`label px-2 py-1 border ${
                    p.status === 'Live' ? 'border-heading text-heading' : 'border-muted'
                  }`}>
                    {p.status}
                  </span>
                </div>
              ...
              <div className="flex flex-row md:flex-col gap-5 items-start md:items-end md:justify-center">
                {p.live ? (
                  ...
                  <span className="label line-through">Live Site</span>
                )}
