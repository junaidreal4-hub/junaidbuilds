const testimonials = [
  ...
  },
  {
    ...
    date:    'December 2025',
    text:    'He built our MVP from scratch in under 6 weeks — frontend, backend, database, all deployed and working. Highly recommend for any serious project.',
  },
  {
    name:    'Lena B.',
  ...
  return (
    <section id="testimonials" className="section-pad bg-surface overflow-hidden">
      <div className="container-width">
        ...
        {/* Horizontal scroll carousel — like nikolaradeski.com */}
        <div className="flex gap-6 overflow-x-auto pb-4 scrollbar-hide -mx-6 px-6"
             style={{ scrollbarWidth: 'none' }}>
          {testimonials.map((t) => (
            <div
              key={t.name}
              className="flex-shrink-0 w-[320px] md:w-[380px] border border-border p-8 flex flex-col justify-between gap-8 bg-canvas"
            >
              <div>
                {/* Star rating */}
                <div className="flex gap-1 mb-6">
                  {[...Array(5)].map((_, i) => (
                    <svg key={i} className="w-3 h-3 fill-heading" viewBox="0 0 20 20">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                  ))}
                </div>
                <p className="text-body text-sm leading-relaxed">&ldquo;{t.text}&rdquo;</p>
              </div>
              <div className="border-t border-border pt-6">
                <div className="text-heading font-mono text-sm font-bold">{t.name}</div>
